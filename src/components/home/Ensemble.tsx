...
export default function Ensemble({ team: parentTeam }: { team?: any[] }) {
  ...
  useEffect(() => {
    // 🧠 Query Firebase: ONLY grab members where showOnHome is exactly true
    const q = query(collection(db, "team"), where("showOnHome", "==", true));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-y-16 md:gap-y-24 gap-x-8 md:gap-x-12">
          {teamMembers.length === 0 ? (
            ...
            // We slice it to 5 just in case you accidentally set 20 people to "showOnHome: true" 😂
                  ...
                  <div className="relative mb-6 w-full max-w-[180px] aspect-square">
                    ...
                    <div className="relative h-full w-full rounded-full border-4 border-[#2D2D2D] overflow-hidden bg-gray-200">
                      {memberImage ? (
                        ...
                        <div className="h-full w-full flex items-center justify-center font-black opacity-20 text-xl uppercase bg-[#FFF9F0]">
                          🎭
                        </div>
                      )}
                    </div>
