...
export default function Ensemble({ team: parentTeam }: { team?: any[] }) {
  ...
  return (
    <section className="py-24 md:py-40 px-6 bg-[var(--bg-primary)] border-b-[12px] border-[var(--border-primary)]">
      <div className="max-w-7xl mx-auto">
        
        {/* 🎭 Header Section */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 md:mb-32 gap-8">
          <h2 className="font-cinzel text-6xl md:text-9xl font-black uppercase leading-none tracking-tighter text-[var(--text-primary)]">
            The <br/> 
            <span className="bg-[#FF5F5F] text-white px-6 shadow-[8px_8px_0px_var(--border-primary)] inline-block mt-2">
              Ensemble
            </span>
          </h2>
          ...
          <Link href="/team" className="group relative inline-block">
            ...
            <div className="relative z-10 bg-white border-4 border-[var(--border-primary)] px-8 py-4 font-black uppercase tracking-widest rounded-xl group-hover:-translate-y-1 transition-transform text-xs md:text-sm text-[var(--text-primary)]">
              Meet the Full Cast 🎭
            </div>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-y-16 md:gap-y-24 gap-x-8 md:gap-x-12">
          {teamMembers.length === 0 ? (
            <div className="col-span-full text-center font-black uppercase text-[var(--text-primary)]/30 tracking-widest">
              ...
              return (
                ...
                >
                  <div className="relative mb-6 w-full max-w-[180px] aspect-square">
                    ...
                    <div className="relative h-full w-full rounded-full border-4 border-[var(--border-primary)] overflow-hidden bg-gray-200">
                      {memberImage ? (
                        <img 
                          src={memberImage} 
                        ...
                        <div className="h-full w-full flex items-center justify-center font-black opacity-20 text-xl uppercase bg-[var(--bg-primary)] text-[var(--text-primary)]">
                          🎭
                        </div>
                      )}
                    </div>
                  </div>
                  <h4 className="font-black uppercase text-lg md:text-xl text-[var(--text-primary)] group-hover:text-[#FF5F5F] transition-colors leading-tight">
                    {m.name || "Performer"}
                  </h4>
                  <p className="text-[9px] md:text-[10px] text-[var(--text-primary)] font-bold uppercase tracking-[0.2em] opacity-40 mt-1">
                    {m.role || "Member"}
                  </p>
                </motion.div>
              );
