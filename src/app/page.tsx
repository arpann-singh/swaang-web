...
export default function Home() {
  ...
  return (
    <PageTransition>
      ...
      <main className="min-h-screen bg-[#FFF9F0] overflow-x-hidden pt-10 md:pt-12">
        {/* FIX: TICKER */}
        {notices.length > 0 && (
          <div className="fixed top-0 left-0 w-full bg-[#FF5F5F] border-b-4 border-[#2D2D2D] z-[2000] h-10 flex items-center overflow-hidden">
            <div className="animate-marquee-infinite">
              {[...notices, ...notices, ...notices].map((n, i) => (
                <span key={i} className="text-white font-black uppercase text-[10px] md:text-[12px] px-12 border-r-2 border-white/20 whitespace-nowrap">
                   📢 {n.title}: {n.content}
                </span>
              ))}
            </div>
        ...
        <section className="relative h-screen flex items-center justify-center border-b-[12px] border-[#2D2D2D]">
          <div className="absolute inset-0 bg-[#2D2D2D]">
             {homeData.headerVideoUrl && <video autoPlay muted loop playsInline className="w-full h-full object-cover opacity-50" src={homeData.headerVideoUrl} />}
          </div>
          <div className="relative z-10 text-center px-4">
             <h1 style={getStyle('heroTitle', '8rem')} className="font-black text-white uppercase break-words leading-[0.9]">
               {homeData.headerTitle || "SWAANG"}
             </h1>
             <p className="mt-6 font-cinzel text-[#06D6A0] text-xl md:text-3xl font-bold uppercase tracking-[0.5em] drop-shadow-md">
               The Drama Club of SSTC
             </p>
          </div>
        </section>
        ...
        <section className="py-32 px-6 bg-[#FFF9F0] border-b-[12px] border-[#2D2D2D]">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-20">
            <div className="w-full md:w-1/2 relative group">
              ...
              <img src={homeData.founderImage || "https://placehold.co/800x1000?text=The+Director"} className="relative z-10 w-full aspect-[4/5] object-cover border-4 border-[#2D2D2D] rounded-[3rem] grayscale hover:grayscale-0 transition-all duration-700" />
            </div>
            <div className="w-full md:w-1/2">
              <span className="bg-[#FF5F5F] text-white px-4 py-2 rounded-full font-black uppercase text-sm mb-6 inline-block">Directorial Note</span>
              <h2 className="font-cinzel text-5xl md:text-7xl font-black uppercase mb-10 leading-tight">{homeData.founderName}</h2>
              <p className="text-xl md:text-2xl leading-relaxed italic font-medium opacity-80 select-none">"{homeData.founderNote}"</p>
            </div>
          </div>
        </section>
        ...
        {aotm && (
          <section className="py-32 px-6 bg-[#FFD166] border-b-[12px] border-[#2D2D2D]">
            <h2 className="text-center font-cinzel text-6xl md:text-8xl font-black uppercase mb-20 tracking-tighter">Artist of the Month</h2>
            <div className="max-w-4xl mx-auto bg-white border-8 border-[#2D2D2D] p-10 rounded-[4rem] shadow-[20px_20px_0px_#2D2D2D] flex flex-col items-center">
              ...
              <p className="mt-4 text-center font-bold text-gray-600">{aotm.achievement}</p>
            </div>
          </section>
        )}
        ...
        <section id="journey" className="py-32 px-6 bg-[#06D6A0] border-b-[12px] border-[#2D2D2D]">
          ...
          <div className="max-w-4xl mx-auto space-y-12 relative before:absolute before:left-1/2 before:w-2 before:h-full before:bg-[#2D2D2D]">
            {timeline.map((m, i) => (
              <div key={m.id} onClick={() => setSelectedMilestone(m)} className={`relative cursor-pointer flex ${i % 2 === 0 ? 'md:flex-row-reverse' : ''} items-center gap-10`}>
                <div className="w-full md:w-1/2 bg-white border-4 border-[#2D2D2D] p-8 rounded-[2rem] shadow-[10px_10px_0px_#2D2D2D] hover:-rotate-1 transition-transform">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[3000] bg-[#2D2D2D]/95 backdrop-blur-md flex items-center justify-center p-6" onClick={() => setSelectedMilestone(null)}>
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#FFF9F0] border-8 border-[#2D2D2D] w-full max-w-4xl rounded-[3rem] p-10 relative overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <img src={selectedMilestone.photo1 || "https://placehold.co/600x400/FF5F5F/white?text=No+Photo+1"} className="w-full aspect-video object-cover border-4 border-[#2D2D2D] rounded-2xl" />
                  <img src={selectedMilestone.photo2 || "https://placehold.co/600x400/FFD166/2D2D2D?text=No+Photo+2"} className="w-full aspect-video object-cover border-4 border-[#2D2D2D] rounded-2xl" />
                </div>
              </motion.div>
            </motion.div>
          )}
